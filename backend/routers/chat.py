...
async def chat(request: ChatRequest, current_user=Depends(get_current_user)):
    ...
    def generate():
        ...
        system_prompt = (
            ...
            "2. ALWAYS use bullet points. NEVER write paragraphs or walls of text.\n"
            ...
            "User: What are the key concepts in this paper?\n\n"
            "Assistant:\n"
            "## Key Concepts\n\n"
            "- **Distributed systems** are defined as systems where message transmission delay "
            "is not negligible compared to the time between events in a single process [1]\n"
            "- **Partial ordering** of events means it is sometimes impossible to determine "
            "which of two events occurred first in a distributed system [2]\n"
            "- **Logical clocks** are counters that assign a number to each event without "
            "requiring a physical timing mechanism [1][3]\n\n"
            "## Clock Conditions\n\n"
            "- If event *a* happens before event *b* within the same process, then the clock "
            "value of *a* must be less than the clock value of *b* [2]\n"
            "- When a message is sent, the receiver must advance its clock to be greater than "
            "the timestamp on the message [2]\n"
        )
